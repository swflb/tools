apt-get install git mercurial giggle gcc gcc-4.9 g++-4.9 gdb imagemagick gimp nitrogen compizconfig-settings-manager Logisim texmaker texstudio guake conky-all vim nano wget elinks plantuml postgresql-9.4 mlocate dstat nmap autoconf libtool libgtkgl2.0-dev  libgl1-mesa-dev libglu1-mesa-dev lcov gcovr clang-3.7 clang-format-3.7 cppcheck chromium-browser gksu lib32ncurses5-dev ncurses-base libsasl2-dev

# virtualbox opera spotify-client
# my-weather-indicator
#apt-get install nvidia-352

#systemctl reboot

